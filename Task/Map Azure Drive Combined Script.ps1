...
foreach ($drive in $driveArray){
    Set-ScheduledTask -TaskName "Map QB Drive" -TaskDesc "Map the $($drive.Name) drive at logon via New-PSDrive" -Trigger AtLogOn -ScriptContent {
        ...
        if ($connectTestResult.TcpTestSucceeded) {
            # Mount the drive
            New-PSDrive -Persist -Name $drive.Name -PSProvider FileSystem -Root $drive.Value -Scope Global
        } else {
